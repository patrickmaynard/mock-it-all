# Finished todo items (public-facing)

These are mass-imported/wiped from the developer list every so often, so don't put
anything important in here.

## Todo items (finished items -- list get wiped every so often):
...
* x Move old finished todo items into a document called
  finished-todos-public.md
